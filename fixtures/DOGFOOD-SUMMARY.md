...
| 05b | `ops.url-health-check` | (same as 05), skill disabled, **post network-dynamic** | **live** | **4 round-trips, 12 codes; host segment 2/2, zero invented hostnames** | 2 (unfixed) | 0 | — |
...
3. ~~**Capture full first-write text** in FINDINGS.md, not just the rejection summary.~~ — **done** in pass 05b: `fixtures/url-health-check-no-skill-rerun/ATTEMPTS.md` records the complete text of every attempt and every rejection payload, written before/after each write by protocol.
